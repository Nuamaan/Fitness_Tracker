...
function remind(){
  fetch('/reminder').then((response)=>{
  response.json().then(data=>{
    let act=data.activity;
    let date=data.date;
    let target=document.getElementById('targetreminder');
    target.innerText=`Did you ${act} on ${date} ?`;
    let targetdiv=document.getElementById('reminder-container');
